...
const Alert = ({ type, text }) => {
    ...
    return (
        <AnimatePresence>
        <motion.div className="fixed z-50 flex items-centerr justify-center bottom-5 right-5"
         initial="hidden"
         animate="visible"
         exit="exit"
         ...
         transition={{ duration: 0.3, ease: "easeInOut" }}>
            ...
            items-center text-indigo-100 leading-none lg:rounded-full flex lg:inline-flex rounded-md p-5`}>
                <p className={`flex rounded-full ${type === "danger" ? "bg-red-500" : "bg-lavender"
                    } uppercase px-2 py- text-xs font-semibold mr-3`}>{type === "danger" ? "Failed" : "Success"}</p>
